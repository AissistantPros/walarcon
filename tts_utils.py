...
async def stream_tts_realtime_to_twilio(
    ...
    """
    Envía TTS de ElevenLabs a Twilio en tiempo real usando formato mu-law nativo.
    SOLUCION: Usa ulaw_8000 directamente - no necesita procesamiento WAV.
    """
    if not api_key:
    ...
    body = {
        ...
        "output_format": "ulaw_8000",  # ✅ SOLUCION: Formato nativo de Twilio
        "voice_settings": {
            "stability": 0.5,
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(10.0, read=60.0)) as cli:
        async with cli.stream("POST", url, headers=headers, json=body) as resp:
            if resp.status_code != 200:
                logger.error(f"[STREAM] ElevenLabs error {resp.status_code}")
                return

            async for raw_chunk in resp.aiter_bytes(1024):  # Chunks más grandes para mu-law
                if raw_chunk:
                    # ✅ SOLUCION: Envío directo - no procesamiento WAV necesario
                    await send_audio_to_twilio_direct(raw_chunk, websocket_send, stream_sid)
...
async def send_audio_to_twilio_direct(audio_data: bytes, websocket_send: Callable, stream_sid: str):
    """
    Envía audio mu-law directamente a Twilio sin procesamiento WAV.
    """
